...
class DetectedVar:
    ...
    def display_source(self) -> str:
        ...
        if self.service:
            # Si el file_path ya empieza con service, removerlo del sufijo
...
def extract_service_prefix(file_path: str, repo_root: Optional[str] = None) -> Tuple[str, str]:
    ...
    if len(parts) > 1:
        first_dir = parts[0]
        # Casos comunes de subdirectorios de microservicios o apps
        ...
        # Si el repo_root está disponible, verificar si tiene package.json en first_dir
        if repo_root:
...
class EnvVarScanner:
    ...
    def _is_comment(self, line: str, filename: str) -> bool:
        ...
        # Python / Bash / YAML / Dockerfile / .env
    ...
    def _has_fallback(self, line: str) -> bool:
        ...
        # Nullish coalescing (??) o Logical OR (||)
        if "??" in line or "||" in line:
            return True

        # Schemas zod: .default(...) o .optional()
        ...
        # Python: os.environ.get("VAR", default) o os.getenv("VAR", default)
        if re.search(r'os\.(?:environ\.get|getenv)\s*\([^,)]+,\s*[^)]+\)', line):
            return True

        # NestJS: configService.get('VAR', default)
    ...
    def _extract_vars_from_line(self, line: str, filename: str) -> List[str]:
        ...
        # 4. CI/CD (.gitlab-ci.yml, .github/workflows)
        ...
        # 5. Código fuente: Node.js / Next.js / NestJS (process.env.VAR o process.env['VAR'])
        ...
        # 10. NestJS ConfigService: configService.get('VAR')
        for m in re.finditer(r'config(?:Service)?\.get(?:<[^>]+>)?\s*\(\s*["\']([A-Za-z_][A-Za-z0-9_]*)["\']', line):
            found.add(m.group(1))

        # 11. Schemas zod centralizados (config/envServer.ts, config/envClient.ts, src/config/env.ts)
        ...
        # 12. LoopBack 4 config: keys.ts (export const VAR = ... o keys de binding)
    ...
    def scan(self) -> List[DetectedVar]:
        ...
        # 2. Recolectar eliminaciones (para detección de renames)
        ...
        # 4. Agrupar candidatos por nombre único y clasificar
        ...
        for var_name, file_path, line_num, content in added_candidates:
            ...
            # Detección de Fallback y Obligatoriedad
